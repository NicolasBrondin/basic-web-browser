...
// Element 3
const node3 = new HTMLElement("a", {});
node3.setAttribute("innerText", "Hello world");
const domNode3 = new DomNode(node3 as Node);
domNode3.style.color = "blue";
domNode3.style["text-decoration"] = "underline";

divDomNode.addChildNode(domNode3);
divDomNode.addChildNode(imageDomNode);
/*
// Element 4
...
let dom : DomNode[] = [domNode, divDomNode, /*domNode3, imageDomNode4*/];

function renderTextNode(widget: QWidget, n: DomNode, painter: QPainter){
   const brush = new QColor(n.style.color || "black");
   ...
   if(lines){
      lines.forEach((line, index)=>{
         ...
         if(n.style["text-decoration"] == "underline"){
            //const widthPercentage = line.length / numberOfCharacters;
            painter.fillRect(
               n.boundingClientRect.left,
               n.boundingClientRect.top + ((index+1) * characterHeight) + 2,
               characterWidth*line.length,
               1, brush);
         }
      });
   }
...
function render(widget: QWidget, n: DomNode, painter: QPainter){
   if(n.getTagName() === "p" || n.getTagName() === "a"){
      renderTextNode(widget, n, painter);
...
win.addEventListener(WidgetEventTypes.Paint,  () => {
   ...
   dom.forEach((el, index)=>{
      ...
      el.children.forEach((child, index2) =>{
         const previousEl2 = index2  > 0 ? el.children[index2-1] : undefined;
         console.log("child");
         child.updateBoundingClientRect(el, previousEl2);
      });
   })
   dom.forEach((el, index)=>{
